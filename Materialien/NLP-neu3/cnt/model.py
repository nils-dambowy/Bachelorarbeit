...
def predict_re_single_sentence_eng(model, sentence):
    import pandas as pd
...
def predict_re_single_sentence_ger(model, sentence):
    import pandas as pd
    df = pd.DataFrame(columns=["id","design_de"])
    df = df.append({"id":0,"design_de":sentence}, ignore_index=True)
    y_pred = model.predict(df)
    y_pred = y_pred["y"].item()
    return y_pred
